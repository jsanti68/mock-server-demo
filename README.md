...
docker run --rm \
  -p 1080:1080 \
  -v $(pwd)/mockserver/specs:/opt/mockserver/specs \
  -v $(pwd)/mockserver/expectations:/opt/mockserver/expectations \
  mockserver/mockserver:5.15.0 \
  -serverPort 1080 \
  -initializationJsonPath /opt/mockserver/expectations/expectations.json
```   
